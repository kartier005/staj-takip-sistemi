...
function SchoolPanel() {
  ...
  const [formData, setFormData] = useState({ ad: '', sehir: '', tur: '', ogrenci_sayisi: '' });
  ...
  useEffect(() => {
    ...
    if (user.role === 'Supervisor') {
      ...
      if (myPerms.page_permissions.includes('Okul Yonetimi')) {
        setHasPageAccess(true);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await api.get('/api/crud/schools');
  ...
  const handleOpenModal = (item = null) => {
    ...
    else { setEditingId(null); setFormData({ ad: '', sehir: '', tur: '', ogrenci_sayisi: '' }); }
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      if (editingId) await api.put(`/api/crud/schools/${editingId}`, { data: formData });
      else await api.post('/api/crud/schools', { data: formData });
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Bu kaydı silmek istediğinize emin misiniz?')) {
      try { await api.delete(`/api/crud/schools/${id}`); fetchData(); }
  ...
  const columns = [
    { key: 'ad', label: 'Okul Adı' },
    { key: 'sehir', label: 'Şehir' },
    { key: 'tur', label: 'Tür' },
    { key: 'ogrenci_sayisi', label: 'Öğrenci Sayısı' }
  ];
  const fieldLabels = {
    ad: 'Okul Adı',
    sehir: 'Şehir',
    ...
    <div className="flex bg-black min-h-screen">
      ...
      <div className="ml-64 flex-1 flex flex-col">
        ...
        <main className="p-8 flex-1">
          ...
          <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingId ? 'Okulu Düzenle' : 'Yeni Okul'}>
            <form onSubmit={handleSubmit} className="space-y-4">
              {['ad', 'sehir', 'tur', 'ogrenci_sayisi'].map(field => (
                <div key={field}>
                  <label className="block text-[10px] font-bold text-zinc-600 uppercase tracking-[0.2em] mb-1.5">{fieldLabels[field]}</label>
                  <input
                    required
                    name={field}
                    type={field === 'ogrenci_sayisi' ? 'number' : 'text'}
